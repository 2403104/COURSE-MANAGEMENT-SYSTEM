...
app.post('/timeTable', async (req, res) => {
    try {
        ...
        const semester = timetableData.semester;
        const finded=await Timetable.findOne({semester});
        if (!finded){
            const added=await Timetable.create(timetableData)
            console.log(added)
        }else{
            const updated=await Timetable.findOneAndUpdate({semester},timetableData,{new:true});
            console.log(updated)
        }

        res.status(200).json({ message: 'Timetable successfully created/updated!' });
    } catch (error) {
        console.error('Error in /timeTable:', error);
        res.status(500).json({ error: 'An error occurred while creating/updating the timetable.' });
    }
});
